...
### Bundle/Jekyll (Recommended Approach)

**Prerequisites:**
- Ruby 3.0+ and Bundler installed
- Python and pip installed
- ImageMagick installed (`brew install imagemagick` on macOS, `sudo apt-get install imagemagick` on Linux)
...
```bash
bundle update --all                    # Update all gems
...
```bash
bundle exec jekyll serve
# Let it build (wait 10-30 seconds)
# Visit http://localhost:4000 and verify site renders correctly
# Exit with Ctrl+C
```
...
Kill the existing Jekyll process: `lsof -i :4000 | grep LISTEN | awk '{print $2}' | xargs kill`
